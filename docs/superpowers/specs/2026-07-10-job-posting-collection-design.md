...
[정규화 (Set/Code)]     공고 → {회사, 제목, 링크, 경력, 기술스택, 공고ID, 마감일}로 통일
      ↓
[Dedup 필터]           이미 본 공고ID 제외 (4-a 참조) — AI 호출 전에 필터링
      ↓
[AI 요약 (OpenAI 노드→로컬)]  공고당 3줄 요약 + 핵심 자격요건 추출
      ↓
...
### (c) 데이터 소스 (원티드/점프짓)
- SPA가 내부적으로 호출하는 JSON 엔드포인트를 사용(HTML 파싱 아님).
- 브라우저 개발자도구 → Network 탭에서 공고 목록 JSON 요청을 찾아 HTTP Request 노드로 재현.
- **주의**: 비공식 엔드포인트라 사이트 개편 시 깨질 수 있음 → 정규화 노드에서 필드 매핑을 한 곳에 모아 유지보수 지점을 최소화.
